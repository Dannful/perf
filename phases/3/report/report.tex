...
A Figura \ref{fig:exec_time_exp1} corrobora o comportamento assintótico ($O(n)$) da aplicação. A linearidade observada é rigorosa, fortemente fundamentada pela tênue variabilidade. Ademais, distingue-se o Tempo de Computação (linha sólida) do Tempo Total (linha tracejada): o Tempo Total inclui a escrita em disco dos resultados, operação cujo custo escala proporcionalmente com o tamanho do problema e o número de iterações, explicando o desvio crescente em relação ao tempo de processamento puro.
...
Entretanto, a utilização de tamanhos de problema restritos revela, sobretudo, o custo de \textit{overhead} associado ao paralelismo. Conforme observado na Figura \ref{fig:overhead_exp1}, a sobrecarga percentual é significativamente alta para entradas pequenas. Em tais cenários, a granularidade, sendo muito fina, acarreta o fato de o tempo gasto criando e sincronizando \textit{threads} represente uma fração significativa do tempo total.
...
A sobrecarga supracitada impõe um limite ao ganho de desempenho. A Figura \ref{fig:speedup_exp1} demonstra que o Speedup se distancia do ideal à medida que o número de threads aumenta. Para os menores problemas ($24^3$), o ganho de velocidade estagna rapidamente. Nota-se, ademais, que o Speedup do Tempo Total tende a ser inferior ao de Computação, pois as operações de I/O incluídas no total são inerentemente seriais ou limitadas pela largura de banda do disco, não se beneficiando do paralelismo da mesma forma que o cálculo numérico.
...
Com base na estabilidade verificada anteriormente, o Experimento 2 estressou a arquitetura com volumes de dados significativamente maiores (até $504^3$). A Figura \ref{fig:exec_time_exp2} demonstra que, mesmo sob carga intensa e com menos replicações, a aplicação mantém seu comportamento linear previsível. Novamente, o Tempo Total apresenta uma inclinação maior que o Tempo de Computação, refletindo o custo de I/O associado à geração de arquivos de saída massivos, proporcionais ao volume da grade simulada.
